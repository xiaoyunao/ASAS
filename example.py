...
def parse_args():
    ...
    parser.add_argument(
        "--fits",
        type=str,
        default="./survey_fov_footprints.fits",
...
def main():
    ...
    # Setup observing site
    site = EarthLocation(lat=args.lat * u.deg, lon=args.lon * u.deg, height=args.height * u.m)

    # Read field footprints
    tbl = Table.read(args.fits, format="fits").to_pandas()
    tbl.columns = [c.lower() for c in tbl.columns]

    # Update exposure history
    history_mgr = HistoryManager(
        history_dir=args.history_dir,
        template_path=args.fits
    )
    history_mgr.update_from_directory(
        directory=str(obs_dir),
